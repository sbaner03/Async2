...
class Con:
    def __init__(self,docknodict,condata):
        ...
        self.a=condata #db.master.find_one({'origin':self.currloc,'destination':self.destination})
    ...
    def geteta(self):
        [self.a["legdata"].get(str(i)).append({'depdetails':self.func(self.a["legdata"].get(str(i))[2],self.currtime,etatype="schedule")}) if i==1 else self.a["legdata"].get(str(i)).append({'depdetails':self.func(self.a["legdata"].get(str(i))[2],self.a['legdata'].get(str(i-1))[3]['depdetails'][2])}) for i in range(1,len(self.a['legdata'])+1)]
        eta = list(self.a['legdata'].values())[-1][3]['depdetails'][2]
        return eta
